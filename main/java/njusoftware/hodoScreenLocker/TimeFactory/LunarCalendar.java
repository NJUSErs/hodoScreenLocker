...
final class LunarCalendar {
    /* 农历数据 */
    static long[] lunarInfo = new long[] { 0x04bd8, 0x04ae0, 0x0a570, 0x054d5, 0x0d260, 0x0d950, 0x16554, 0x056a0,
    ...
    static String chineseNumber[] = { "一", "二", "三", "四", "五", "六", "七", "八", "九", "十" };
    static String chineseNumber1[] = { "正", "二", "三", "四", "五", "六", "七", "八", "九", "十", "十一", "腊" };

    static String getLunarCalendar(Calendar cal) {
        int[] temp = calculate(cal);
        return cyclicalm(temp[0]-1864) + " " + animalsYear(temp[0]) + " " + (temp[3] == 1 ? "闰" : "")
                + chineseNumber1[temp[1] - 1] + "月" + getChinaDayString(temp[2]);
    }

    static int[] calculate(Calendar cal) {
        int offset = (int) ((cal.getTime().getTime() + 2206425943000L) / 86400000L);
        ...
        return new int[] { year, month, offset + 1, leap ? 1 : 0 };
    }
